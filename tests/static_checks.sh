...
assert_grep 'install -m 0755 "\$src" /usr/local/bin/wdtt-xray-rules.sh' "$INSTALL" "upgrade installs refreshed helper"
assert_grep 'install -m 0755 "\$src" "\$\{INSTALL_DIR\}/templates/wdtt-xray-rules.sh"' "$INSTALL" "upgrade refreshes /usr/local/wdtt/templates helper"
