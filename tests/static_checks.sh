...
assert_grep 'INSTALLER_VERSION="1\.5\.74"' "$INSTALL" "INSTALLER_VERSION=1.5.74"
assert_grep 'installer v1\.5\.74' "$README" "README installer version"
...
assert_grep 'WDTT_XRAY_RULES_BIN:-/usr/local/bin/wdtt-xray-rules.sh' "$INSTALL" "helper dest override for tests / non-root"
assert_grep 'install_file_unless_same' "$INSTALL" "skip GNU install when src is dest (issue #51)"
assert_grep 'copy_tree_unless_same' "$INSTALL" "skip cp when templates already live in INSTALL_DIR"
assert_grep 'WDTT_SCRIPT_DIR' "$INSTALL" "script_dir override for piped/update tests"
assert_not_grep '^IFACE="wdtt0"$' "$XRAY" "template is not the legacy wdtt0-only helper"
assert_grep 'UDP TPROXY' "$XRAY" "template applies inner UDP TPROXY"
assert_not_grep 'if \[ ! -x /usr/local/bin/wdtt-xray-rules.sh \] && \[ -f /usr/local/wdtt/templates/wdtt-xray-rules.sh \]' "$INSTALL" "xray unit must not keep a stale helper just because it is executable"
assert_grep 'if \[ -f /usr/local/wdtt/templates/wdtt-xray-rules.sh \]; then /usr/bin/install -m 0755 /usr/local/wdtt/templates/wdtt-xray-rules.sh' "$INSTALL" "xray unit always copies packaged helper"
