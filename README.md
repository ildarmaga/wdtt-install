...
В шапке установщика: **installer v1.5.73** (линейка релизов [ildarmaga/wdtt](https://github.com/ildarmaga/wdtt)).
...
Если `wdtt update` пишет «не удалось получить список версий» — обычно rate limit / 403 GitHub API с VPS. Обход: `export GITHUB_TOKEN=...` или `WDTT_VERSION=v1.5.73 wdtt update` (тег всегда с префиксом `v`).
